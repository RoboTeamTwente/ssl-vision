...
private:
    ...
    int g_whiteMargin = 140;                // minimum color threshold for a pixel to be determined 'white'
    int g_deltaWhiteMargin = 15;            // after one pixel has been found, lower the minimum threshold by this amount
    ...
    bool findMarkerId(std::vector<bool> &resultData, std::vector<int> &markerIds);
